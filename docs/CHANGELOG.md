...
All notable changes to Enpour will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).
Enpour uses [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
...
- **Push notifications for schedule updates** — `push_tokens` table (migration `20260715000001`) stores one Expo push token per device, keyed by unique token with upsert-on-conflict so device reassignment never duplicates rows; RLS restricts read/write to the owning user, with no cross-user `SELECT` policy — the delivery function reads via the service role instead; `types/PushToken.ts`, `services/pushTokenService.ts` (`upsertPushToken`), `lib/notifications.ts` (permission check/request, Expo push token retrieval, web/no-device guarded), `hooks/usePushNotifications.ts` (registers a token for any authenticated session — guest or professional — and handles notification-tap deep links via `expo-router`); mounted once from `app/_layout.tsx`
- **`supabase/functions/notify-schedule-update`** — Edge Function invoked after a successful schedule save; derives the professional strictly from the caller's verified JWT (never a client-supplied id), looks up guest followers via `follows`, resolves their push tokens via the service role, and sends a batched request to the Expo Push API; deletes tokens that come back `DeviceNotRegistered` in the same pass; receipt polling deferred as a future enhancement
- **`services/notificationService.ts`** — `notifyScheduleUpdate()`; called from `useSchedule.save()` after a successful save, fire-and-forget — delivery failures never surface as a save failure
...
- **Push notifications require a development build** — `expo-notifications` remote delivery cannot be exercised in Expo Go (unavailable on Android from SDK 53; Expo Go's shared bundle identifier makes it unsuitable on iOS regardless). A development build additionally needs an iOS bundle identifier / Android package name, which are still undecided (same blocker as Universal Links above), plus an Apple Push Notifications key and Android FCM v1 credentials, neither of which exist yet.
...
- **Custom domain postponed, reverted to EAS Hosting alias** — a real-world QR code scan led to a broken page; diagnosed as `enpourapp.com` returning a Cloudflare 522 (proxy/orange-cloud enabled against Expo's documented DNS-only guidance, origin unreachable) for every path, unrelated to app code, the `/pro/[id]` route, static export, or RLS (all separately verified working). `EXPO_PUBLIC_PROFILE_BASE_URL` reverted to `https://tapindev.expo.app` in `.env` and the EAS `production` environment variable; redeployed via `eas deploy --prod` and reverified against two real professional profiles (logged-in QR/link generation and the logged-out "Sign up to Follow" → guest signup `returnTo` flow) plus the Supabase Auth redirect. `enpourapp.com` and its Supabase redirect URL entry are left in place, unremoved, for when the custom domain is resumed — see [`docs/architecture/domain-and-hosting.md`](../architecture/domain-and-hosting.md) for the DNS-only fix and resumption steps.
